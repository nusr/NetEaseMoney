...
export default class deviceStorage {
  ...
  /**
   * Updates the value in the store for a given key in AsyncStorage. If the value is a string it will be replaced. If the value is an object it will be deep _.merged.
  ...
  update(key, value) {
    return this.get(key).then(item => {
      const temp = _.isString(value) ? value : _.merge({}, item, value);
      return AsyncStorage.setItem(key, JSON.stringify(temp));
    });
  }
